...
public class PruebaInteractiva5 {
	...
	public String run (String c) {
		...
		if (c.indexOf(cmds[i].split(" ")[0]) == 0) { // "selenpeligro"
			String r = "";
			Object[] objectArray = this.cc.getDrones().toArray();
			Dron[] dronArray = Arrays.copyOf(objectArray, objectArray.length, Dron[].class);
			SelectorMision sm = new SelectorMisionDronEnPeligro(dronArray);//(Dron[])this.cc.getDrones().toArray());
			for (Mision m: this.cc.getMisiones(sm))
				r += m.toString()+ "\n";
			return r + "\n";
		}
